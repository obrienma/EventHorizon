...
See: docs/journal/eventhorizon-2026-06-15T0900-custom-otel-metrics.md
...
type: basic
deck: Rhizome::EventHorizon
tags: [eventhorizon, phase-18, observability, debugging]
cross-ref: observability
---
Q: `events.dead` held 108 messages while `events_failed_total` read 0 — an apparent contradiction. What caused the gap, and why is it "a feature, not a bug"?

A: `events.work` carries both an `x-message-ttl` of 30s and a dead-letter exchange, so there are two independent routes into `events.dead`: the worker's explicit `nack` after exhausting retries (`reason=rejected`, which increments `events_failed_total`) and broker-level TTL expiry of messages never consumed in time (`reason=expired`, which runs no application code and is never counted). The 108 were backlog that accumulated and expired while the worker was down. `events_failed_total` is the processing-failure subset of dead-letters, never the total; `rabbitmq_queue_messages{queue="events.dead"}` is the complete signal, and the gap between the two is exactly the expired backlog — a counter and a queue depth that "should" agree but measure different paths is the dashboard working as designed, not a defect.

Extra: EventHorizon · Phase 18 · Challenge: DLQ Depth and events_failed_total Disagree By Design
See: docs/journal/eventhorizon-2026-06-15T0900-custom-otel-metrics.md
