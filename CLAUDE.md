...
**Phase 25 (2026-07-14):** GKE deployment prep. `docs/adr/0020` resolves two questions Xylem-L6's ADR 0003 had left open for EventHorizon specifically: manifest approach (raw manifests stay raw — existing `k3s/*.yaml` is already GKE Autopilot-compatible, no Helm chart introduced) and RabbitMQ placement (in-cluster, in the `event-horizon` namespace, not an external managed vendor). Added `k3s/rabbitmq.yaml` — single-replica Deployment + `ReadWriteOnce` PVC + ClusterIP Service named `rabbitmq`, mirroring `docker-compose.yml`'s broker exactly (same image `rabbitmq:3-management-alpine`, same three ports, same guest/guest credentials) and satisfying the `rabbitmq.event-horizon.svc.cluster.local` DNS name `configmap.yaml` already assumed. `strategy.type: Recreate` set explicitly to avoid a rollout deadlock against the singly-mounted PVC (Kubernetes' default `RollingUpdate` would start the new pod before the old one releases the volume). `k3s/secret.yaml`'s `RABBITMQ_URL` filled in with the real (non-sensitive) base64-encoded in-cluster connection string; the Mongo credential field (originally `MONGO_URI`, since replaced by `MONGO_PASSWORD` — see Phase 26) stays an empty placeholder as a real per-environment secret. No `kubectl` available in this environment — manifests validated as syntactically-correct YAML only, not dry-run/schema-validated against a live API server; that verification is still outstanding. Rhizome Lens's own manifest-approach and RabbitMQ-adjacent questions remain unresolved and out of scope for this ADR.

**Phase 26 (2026-07-14):** MongoDB Atlas config + RabbitMQ credential split (`docs/adr/0021`). The user provisioned a MongoDB Atlas cluster and is deploying to GKE — `src/config.ts` now accepts either a full `MONGO_URI` (local docker-compose, unauthenticated) or discrete `MONGO_HOST`/`MONGO_USERNAME`/`MONGO_PASSWORD` (Atlas), enforced by a Zod `.refine()` requiring one complete shape or the other; the resolved connection string is exported as `config.MONGO_URI` either way, so `src/storage/db.ts` needed no changes. Live-verified against the real Atlas cluster (`connectDb()` → `closeDb()` succeeded end-to-end via a scratch script, since `tsx -e` doesn't support top-level await). The same split-and-derive pattern was then applied to RabbitMQ: manual edits to `k3s/rabbitmq.yaml` caught that RabbitMQ hardcodes a localhost-only restriction on the literal `guest` username for every protocol, which would reject all pod-to-pod AMQP traffic in-cluster (invisible locally since docker-compose reaches the broker via loopback) — fixed with a dedicated `RABBITMQ_USER`/`RABBITMQ_PASSWORD` pair. `config.ts` now derives `RABBITMQ_URL`/`RABBITMQ_MANAGEMENT_URL` from `RABBITMQ_HOST` (`configmap.yaml`) + that credential pair (`secret.yaml`), the same optional-fields-plus-refine shape as Mongo, eliminating four credential-encoding fields down to two. `k3s/secret.yaml` is now gitignored — renamed the tracked template to `k3s/secret.example.yaml` (`.env`/`.env.example` pattern), updated `docs/DEV_GETTING_STARTED.md` to `cp` it first. Caught and reconciled a divergence where manual edits to the now-gitignored local `secret.yaml` had reverted the Mongo split back to a plain `MONGO_URI`, out of sync with `secret.example.yaml`'s template — gitignoring a secrets file removes the commit-time safety net but also the diff-time visibility net, so it needed an explicit key-set cross-check instead. Full end-to-end verification: simulated the exact `envFrom` merge of `configmap.yaml` + a filled-in `secret.yaml`, isolated from the real `.env` (a first attempt was contaminated by `dotenv/config`'s silent cwd-relative backfill), confirming both `MONGO_URI` and `RABBITMQ_URL`/`RABBITMQ_MANAGEMENT_URL` resolve correctly. Suite still 44/44 green; `tsc --noEmit` clean. Also caught and flagged a redaction-filter miss that briefly exposed the real Atlas password in the conversation transcript while inspecting `.env` — user was advised to rotate that credential.

**Phase 27 (2026-07-19):** Reversed ADR 0021 — in-cluster MongoDB, not Atlas, per `docs/adr/0023`. A full day of GKE-to-Atlas connectivity investigation (three Atlas clusters, two cloud providers) confirmed every connection attempt failed identically with `SSL routines:ssl3_read_bytes:tlsv1 alert internal error` immediately after `ClientHello`; a temporary GKE Standard cluster with nodes given direct external IPs — structurally removing Cloud NAT from the path — connected on the first attempt with no other change, confirming Cloud NAT (not TLS version, SNI, IP allowlist, MTU, or IP reputation, all individually ruled out) as the mechanism. Atlas private connectivity (VPC Peering/PSC/PrivateLink) would sidestep NAT directly but requires M10+ (~$57+/month), breaking this project's free-tier-first posture for an unconfirmed fix. Added `k3s/mongodb.yaml` (single-replica Deployment + PVC + ClusterIP Service, `mongo:7 --replSet rs0 --bind_ip_all`, unauthenticated, `Recreate` strategy) mirroring `k3s/rabbitmq.yaml`'s Phase 25 shape exactly. `k3s/configmap.yaml`/`configmap.example.yaml`: `MONGO_HOST`/`MONGO_USERNAME` replaced with `MONGO_URI` pointing at the in-cluster Service DNS name. `k3s/secret.yaml`/`secret.example.yaml`: `MONGO_PASSWORD` removed — no Mongo secret exists anymore. No `src/config.ts` changes — the unauthenticated-`MONGO_URI` code path already existed for local dev, so this was a manifest-only reversal. Also fixed a loose end from Phase 26: `k3s/secret.yaml` had been added to `.gitignore` but never actually `git rm --cached`, so it was still tracked and the real credentials filled in locally were sitting in `git status` as a stageable change — untracked it (`git rm --cached`, file kept on disk). ADR 0021 stays `Accepted` as a historical record (never edit accepted ADR text) with a `Superseded by` addendum pointing at ADR 0023.
